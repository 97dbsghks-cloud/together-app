  ...
  return (
    <div className="flex-1 flex overflow-hidden p-4 gap-3" style={{ background: 'var(--t-bg)' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 gap-3">
        ...
        {/* 하단 — 프로젝트 진행률 + 팀 활동 (채팅 자리) */}
        <div className="flex gap-3 flex-shrink-0" style={{ height: 220 }}>

          {/* 프로젝트 진행률 */}
          <div className="flex-1 flex flex-col min-w-0" style={cardStyle}>
                ...
                return (
                  <div key={proj.id} className="px-2 py-1.5 rounded-xl cursor-pointer transition-colors t-hover" onClick={() => onSelectProject?.(proj.id)}>
                    ...
                    <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--t-border)' }}>
                      <div className="h-full rounded-full transition-all duration-700" style={{ width: `${pct}%`, background: pct === 100 ? '#34c759' : 'linear-gradient(90deg, #6366f1, #8b5cf6)' }} />
                    </div>
          ...
          <div className="flex-1 flex flex-col min-w-0" style={cardStyle}>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-1">
              {activityFeed.length === 0 ? (
                ...
                <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-xl t-hover cursor-pointer transition-colors" onClick={() => onSelectProject?.(item.projectId)}>
